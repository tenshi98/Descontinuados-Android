...
import info.androidhive.supegatrab.app.ServiceLocation;
import info.androidhive.supegatrab.helper.DB_usuarios;
import info.androidhive.supegatrab.helper.DB_notificaciones;
...
public class MainActivity extends AppCompatActivity implements FragmentDrawer.FragmentDrawerListener {
	...
	private void displayView(int position) {
		...
		switch (position) {
			...
			case 3: fragment = new View_Seguridad_Fragment();          break;
						...
						if (!error) {
							...
							try {
								...
								for (int i = 0; i < notificaciones.length(); i++) {
									...
									String idTrabajos   = sub.getString("idTrabajos");
									String Latitud      = sub.getString("Latitud");
									String Longitud     = sub.getString("Longitud");
									String idTipoNoti   = sub.getString("idTipoNoti");


									//agrego los contactos a la base interna
									dataNotificaciones.addNoti(new Notifications(mensaje,texto,Fecha,Hora,Web,tipoNoti,room,idTrabajos,Latitud,Longitud,idTipoNoti ));
	...
	/**********************************************************************************************/
	//Se solicita la ubicacion
	public void  disponibilidad(final String idCliente, final int  idDisponibilidad) {
		...
		try {
			StringRequest strReq = new StringRequest(Request.Method.POST,
				...
				@Override
				protected Map<String, String> getParams() {
					String s = String.valueOf(idDisponibilidad);

					// Posting parameters to login url
					Map<String, String> params = new HashMap<String, String>();
					params.put("idCliente", idCliente);
					params.put("idDisponibilidad", s);
